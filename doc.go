...
// fingerprinter holds state that no lock guards, so two goroutines that share one
// instance write a data race. The race detector reports that race, and the library
// detects it at no point.
//
