...
- Four exported names, which emit the JA4SSH window that one connection holds open and then
  remove that connection: `WindowCloser.CloseConnectionWindow`,
  ...
  spec file numbers the method as a requirement, and issue #268 reads whether the method
  stays on the `WindowCloser` interface.
