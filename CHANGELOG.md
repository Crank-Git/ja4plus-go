...
- Five exported names, which emit the JA4SSH window that one connection holds open and then
  ...
  `SyncProcessor.CloseConnectionWindow`, and `ConnectionWindowCloser.CloseConnectionWindow`. The maintainer ruled the name on 2026-08-12, and
  ...
  spec file numbers the method as a requirement, and the maintainer ruled on issue #268 that the method sits on
  its own interface. `WindowCloser` keeps `CloseOpenWindows` alone, and each dispatch asserts
  one capability, so a type that implements one method alone is never skipped.
