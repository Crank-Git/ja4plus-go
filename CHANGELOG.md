...
the current tree
reports 1754 matches, 264
deviations, 588 accepted deviations and 620 register keys. The run also reports 175 unaccepted
uncovered values and 32 accepted uncovered values, and #361 states what an uncovered value is.
An accepted deviation and an accepted uncovered value each name one register entry, so 588 and
32 add up to the 620 register keys. A count that an entry below states therefore differs from
a fresh run.
