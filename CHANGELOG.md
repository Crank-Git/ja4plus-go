...
the current tree
reports 1753 matches, 276
deviations, 577 accepted deviations and 597 register keys. The run also reports 184 unaccepted
uncovered values and 20 accepted uncovered values, and #361 states what an uncovered value is.
An accepted deviation and an accepted uncovered value each name one register entry, so 577 and
20 add up to the 597 register keys. A count that an entry below states therefore differs from
a fresh run.
