...
func directDecodeCallSite(t *testing.T) map[string][]int {
	...
	// The walk reads the git index rather than the worktree alone. A test of the root
	// package writes a temporary build directory into the repository root, and that
	// directory holds a `main.go` of its own. The two packages run as two processes, so
	// the directory can appear and vanish under this walk. A tracked path holds neither
	// risk. `trackedFilesForOwnedCounts` states why an environment without git skips.
