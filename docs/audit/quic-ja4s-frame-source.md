...
page states the question in its `## What this reading does not answer` section, first bullet:
which frame supplies the library's JA4S value for the connection `0:50280` of
...
`## The register entry each answer implies` below states what each one holds.
...
## The connection, and the five frames that carry its handshake
...
handshake message of the QUIC connection.
...
49.** It carries the client's original Destination Connection ID `203f9e9f68698274`, and
`JA4SFingerprinter` holds that identifier in `quicDCIDs` until the server answers.
...
|---|---|---|
| `JA4SFingerprinter` | `ProcessPacket` reads `quicDCIDs`, then `ParseQUICServerInitial` calls `DeriveServerInitialKeys` with the client's original Destination Connection ID. | The key that protects the packet. |
...
**A server Initial packet of this connection carries a zero-length Destination Connection
ID.** `tshark` measured each identifier of the three frames:
...
**So `DecryptQUICInitialCrypto` derives the client role key of an empty connection
identifier, and one identifier protects every Initial packet of the connection.** RFC 9001
Section 5.2 states the identifier:
...
**The error names a cipher, a key or a nonce, and it is the key.** The cipher is
AES-128-GCM on both paths, and RFC 9001 Section 5.1 states that cipher for
`TLS_AES_128_GCM_SHA256`. The nonce derives from the same wrong secret as the key, so one
fault produces both. **The `message authentication failed` text is the report of the Go
...
2. **The protocol character differs**, and one function writes it. `computeJA4SPair` writes
   `q` for `IsQUIC`, and `ParseQUICServerInitial` is the one function that sets `IsQUIC`. A
   value that read the TCP ServerHello of frame 6 would carry `t`.
...
decryption secret at all. The probe of answer 3 proves it: two frames of the capture produce
the whole value.
...
## The register entry each answer implies
...
**This page writes neither entry**, and #484 owns `testdata/deviations.json`.
...
`ja4plus/fingerprinters/ja4s.py:154-157` collects the CRYPTO fragments of a server Initial
with the client's Destination Connection ID. The reading is of the port at the tag `v1.1.0`.
...
**The two counts are equal**, and the run that produced them is the `make conformance` run of
`issue/496-quic-ja4s-frame-source`.
