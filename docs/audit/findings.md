...
| F-24-13 | `lookup.go` | 55 | major | unconfirmed | A cache file whose first row is not valid CSV leaves the table empty, so `LookupFingerprint` returns no result for every fingerprint. |  | Issue #24 measured that the loader ran once for each process through `lookupOnce`, so no test of that audit reached it with a crafted cache file. #74 replaced that loader on 2026-08-14, and `## What Epic 9 changed under S2, S3 and F-24-13 through F-24-15` below names the test that now reaches it. |
...
and `activeTable` of `lookup.go` publishes one immutable snapshot for every reader.
