...
- **FR-gaps-14d** — `JA4Fingerprinter` and `JA4SFingerprinter` each hold an index from the
  reported key to the grouping key, so `CleanupConnection` removes a tunneled connection.
