...
by hand. **The second fact: no exported function accepts a `KeyLog`.** So no `Processor`
method and no fingerprinter reads a secret, and FR-gaps-17c states that fact.

**Issue #492 needs a key log that a fingerprinter reads, and FR-gaps-17c states that no
...
API, so a `KeyLog` parameter needs a ruling of the maintainer.** This amendment states no
roadmap, and issue #171 is its reversal path.
...
- This feature set changes the API at one place. FR-gaps-17 through FR-gaps-17b add the
  eight exported names, and FR-gaps-17c states that no exported function accepts a `KeyLog`.
