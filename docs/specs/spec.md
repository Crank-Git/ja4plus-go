...
| `CloseConnectionWindow` | `JA4SSHFingerprinter` exports it, and `Processor` and `SyncProcessor` each export the wrapper. `ConnectionWindowCloser` declares it. | No change here. **This project named the behaviour first**, so rule 2 runs the other way and the port adopts the name. The port holds no per-connection flush, and round 27 of the `## Changelog` below records that reading. This row moves no fingerprint. | 2 | #216, `Crank-Git/ja4plus#598` |
