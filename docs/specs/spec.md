...
| Item | This project today | What must change | Rule | Ruling |
|---|---|---|---|---|
| JA4LS | **Implemented.** `ja4l.go:183` and `ja4l.go:381` each emit the server value, and `ja4l.go:493` sets `Type: "ja4l"` for every result. **The row read `Not implemented` and cited `ja4l.go:198` until 2026-08-13, and both statements were wrong.** | Done. **The type stays `ja4l`, and the `JA4L-S=` label names the method.** The maintainer ruled on 2026-08-13 that JA4LS reaches no type of its own, because one connection has one owner. `ja4plus/processor.py:106` at `v1.1.0` registers `("ja4l", JA4LFingerprinter)`, so the port writes one type for both methods. | 1 | `features/12-ja4ls.md`, and this repository's #60 |
...
| Subfield 1 of JA4D6 on a relay message | **Writes the outer DHCPv6 message type alone.** #57 measured it at `ja4d6.go:138-141`, which reads `dhcp.MsgType`. A one-level relay gives `rlayf0000nn_9_00` and a two-level relay gives the same value. | Done. No change to the code. The dissector writes both the outer and the inner type, which gives sixteen characters. `ja4d6_relay_message_test.go` holds the separating packets, and it builds the two-level case. **Part b, part c and subfield 2 of part a read no option of the inner message, and the port reads them at any nesting depth.** No requirement of Epic 8b names that difference, and #57 reports it rather than building it. | 1 | #271 |
...
| `JA4XFingerprinter` | `reassembler`, `processedCerts`, `certsByStream`, `streamBytes`, `lastCleanup`, guarded by `mu` | Five-tuple, certificate hash | `Reset`, `CleanupConnection`, an interval sweep |
...
and `internal/parser/tls.go` already wrote it. **No behaviour changed.**
...
**So the ratified behaviour rests on a stated rule.** The maintainer ratified `00` on
