...
| 10 | 2026-08-11 | The maintainer accepted eight exported names before the Epic 10 freeze: `ErrNoSecret`, `KeyLog`, `ParseKeyLog`, `ReadKeyLogFromCapture`, `KeyLog.Secret`, `KeyLog.ClientRandoms`, `KeyLog.Len` and `DecryptQUICPacket`. #40 of Epic 5 builds them for FR-gaps-15 through FR-gaps-18, and this is the first slice of the session to widen the exported surface. The `Interface` row of the register records the parity direction: this project shipped the key-log interface first, the port holds no such interface, and the port adopts these names under rule 2. The `Terms` table gains `key log`, `secret` and `decryption secrets block`. |
