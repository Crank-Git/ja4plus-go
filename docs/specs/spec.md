...
| 9 | 2026-08-11 | The maintainer ruled that the CI conformance job of #36 reports and does not block. FR-conformance-34 and FR-conformance-35 change meaning for that slice. The job runs the suite and publishes the counts in the job summary. A deviation does not fail the job. #33 measured 302 matches and 1858 deviations over 38 captures against an empty register. A job that blocks stays red on every pull request until Epic 5 closes the list, and a permanently red job teaches every reader to ignore it. Epic 5 flips the job to blocking. FR-conformance-36 holds without a change: the job fails when the corpus is absent, because a skipped suite that reports success hides every deviation. |
