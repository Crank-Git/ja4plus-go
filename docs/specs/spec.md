...
|---|---|---|
| `key` | string | The capture, the stream and the method. The middle part holds the stream number in the per-stream set, and the frame number in the per-packet set. The register holds each key once. |
