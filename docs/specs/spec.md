...
| delegated ruling | noun | One ruling the project manager makes where a published FoxIO value contradicts a published FoxIO rule. A schema violation has one right answer, and a reference split has none. The project manager rules the first, and never the second. A delegated ruling stays provisional until the maintainer confirms it, and a later reader reads an unconfirmed one as unconfirmed. | automatic ruling, agent ruling, provisional decision |
