...
| cross-member review | noun | One read-only review of the merged result of a whole batch, which looks for a defect that no member reaches alone. `.claude/rules/cross-member-review.md` states the return contract. It names the term in full at the head of each section, and `review` inside that file names a cross-member review and never an audit. | batch review, integration review, merge review |
| mailbox | noun | The channel one session addresses a cross-session agent through. A spawn response that names a mailbox reached the cross-session path. | inbox, message queue, channel |
